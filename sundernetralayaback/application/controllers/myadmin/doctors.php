...
class Doctors extends CI_Controller{
	...
	public function __construct(){
		...
		$this->OOZPGINFO["TBL"]='doctors';
		$this->OOZPGINFO['PK']='id';
		$this->folder_name='doctors';
	}
...
public function fetchdata(){
        ...
        $select='*';
		$where_like="(designation LIKE '%".$sSearch['value']."%' or name LIKE '%".$sSearch['value']."%')"; 
		...
		foreach($result['data']as $key=>$row){
			...
			$sub_array[]=$cnt;
			$sub_array[]='<img src="'.base_url(DS.$row->image).'" width="40" height="40">';
			$sub_array[]=$row->name;
