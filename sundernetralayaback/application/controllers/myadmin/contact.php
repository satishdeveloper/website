<?php if(!defined('BASEPATH'))exit('No direct script access allowed');class Contact extends CI_Controller{
	...
	public function __construct(){
		...
		parent::__construct();$this->load->library('upload');$this->load->model('master_model');$this->OOZPGINFO["TBL"]='lead_master';$this->OOZPGINFO['PK']='leadID';$this->folder_name='contact';}
...
$select='*';
$where_like="(leadName LIKE '%".$sSearch['value']."%' or leadMobile LIKE '%".$sSearch['value']."%' or leadEmail LIKE '%".$sSearch['value']."%')";
...
if(strpos($currentURL,$trash)!==false){
	...
	foreach($result['data']as $key=>$row){
		$sub_array=array();
		$sub_array[]='<input type="checkbox" id="checkbox_del[]" name="checkbox_del[]" class="flat" value="'.$row->leadID.'">';
		$sub_array[]=$cnt;
		$sub_array[]=$row->leadName;
		$sub_array[]=$row->leadMobile;
		$sub_array[]=$row->leadEmail;
		$sub_array[]=$row->leadMessage;
		$edit_option='';
		$sub_array[]=$edit_option.'&nbsp;&nbsp;<a href="'.base_url('myadmin/'.$this->router->fetch_class().'/view/'.$row->leadID).'" title="Click here for View"><i class="fa fa-eye"  style="font-size: 25px;"></i></a>';
		$data[]=$sub_array;
		$cnt++;
	}
	$result['data']=$data;
