...
class Patient_stories extends CI_Controller{
	...
	public function __construct(){
		...
		$this->OOZPGINFO["TBL"]='testimonials';
		$this->OOZPGINFO['PK']='id';
		$this->folder_name='testimonials';
	}
...
public function fetchdata(){
        ...
        $select='*';
		$where_like="(name LIKE '%".$sSearch['value']."%' or designation LIKE '%".$sSearch['value']."%' or video_link LIKE '%".$sSearch['value']."%')"; 
		...
		foreach($result['data']as $key=>$row){
			...
			$sub_array[]=$cnt;
			$sub_array[]='<img src="'.base_url($row->image).'" width="40" height="40">';
			$sub_array[]=$row->name;
			$sub_array[]=$row->designation;

			$edit_option='<a href="'.base_url('myadmin/'.$this->router->fetch_class().'/edit/'.$row->id).'" title="Click here for Edit"><i class="fa fa-edit"  style="font-size: 25px;"></i></a>';
		...
		$config=array('upload_path'=>$image_path.DS,'allowed_types'=>'jpg|jpeg|gif|png','file_name'=>rand(100000,999999),'max_size'=>0);
			...
			if($_FILES['image']['name']!=''){
				if($this->upload->do_upload('image')){
					$dt=$this->upload->data();
					$_POST['image']=$image_path.'/'.$dt['file_name'];
				}else{
					$_POST['image']=$_POST['old_blogImage'];
