...
class Specialities extends CI_Controller{
	...
	public function __construct(){
		...
		$this->OOZPGINFO["TBL"]='specialities';
		$this->OOZPGINFO['PK']='SPID';
		$this->folder_name='specialities';
	}
...
public function fetchdata(){$sSearch=$this->input->post('search');$table_name=$this->OOZPGINFO['TBL'];$select=''.$this->OOZPGINFO['TBL'].'.SPID,
		'.$this->OOZPGINFO['TBL'].'.name,
		'.$this->OOZPGINFO['TBL'].'.synopsis, 
		'.$this->OOZPGINFO['TBL'].'.status,
		'.$this->OOZPGINFO['TBL'].'.image_url,
		'.$this->OOZPGINFO['TBL'].'.description';
		$where_like="(synopsis LIKE '%".$sSearch['value']."%' or name LIKE '%".$sSearch['value']."%')";
		...
		foreach($result['data']as $key=>$row){
			$sub_array=array();
			$sub_array[]='<input type="checkbox" id="checkbox_del[]" name="checkbox_del[]" class="flat" value="'.$row->SPID.'">';
			$sub_array[]=$cnt;
			$sub_array[]='<img src="'.base_url(DS.$row->image_url).'" width="40" height="40">';
			$sub_array[]=$row->name;
			$sub_array[]=$row->synopsis;

			$edit_option='<a href="'.base_url('myadmin/'.$this->router->fetch_class().'/edit/'.$row->SPID).'" title="Click here for Edit"><i class="fa fa-edit"  style="font-size: 25px;"></i></a>';
			$sub_array[]=$edit_option.'&nbsp;&nbsp;<a href="'.base_url('myadmin/'.$this->router->fetch_class().'/view/'.$row->SPID).'" title="Click here for View"><i class="fa fa-eye"  style="font-size: 25px;"></i></a>';
			$data[]=$sub_array;
			$cnt++;
		}
		$result['data']=$data;
		...
		$config=array('upload_path'=>$image_path.DS,'allowed_types'=>'jpg|jpeg|gif|png','file_name'=>rand(100000,999999),'max_size'=>0);
			...
			if($_FILES['image_url']['name']!=''){
				if($this->upload->do_upload('image_url')){
					...
					$_POST['image_url']=$_POST['old_image_url'];
			...
			$tag_url=str_replace(' ', '-', $_POST['name']);
			$tag_url=preg_replace('/[^A-Za-z0-9\-]/', '', $tag_url);
			$_POST["tag_url"]=$tag_url;
			$_POST["status"]=1;
